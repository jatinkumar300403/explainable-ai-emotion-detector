...
TARGET_FPS      = st.sidebar.slider("Target FPS", 5, 30, 20, 1)
...
def to1d(arr):
    """Ensure predictions are 1-D float32 of length 7."""
    a = np.asarray(arr)
    a = np.squeeze(a)
...
if run:
    ...
    if not cap.isOpened():
        st.error("Webcam not found. Close other apps that may be using the camera and try again.")
    else:
